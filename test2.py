...
def get_attribute_ktp(ls_word, field_name, field_keywords, typo_tolerance, debug_mode=False):
    ...
    if(field_name == 'nama'):
        ls_word = np.asarray(
            [word for word in ls_word if word['label'].lower() not in ['jawa', 'nusa']])

    new_ls_word = np.asarray([word['label'].lower() for word in ls_word])

    ls_dist = [levenshtein(field_keywords, word.lower())
    ...
    w = ls_word[index]['w']
    degree = calDeg(ls_word[index]['x1'], ls_word[index]
                    ['y1'], ls_word[index]['x2'], ls_word[index]['y2'])

    ls_y = np.asarray([np.abs(y-word['y1']) < 300 for word in ls_word])

    value_words = [ww for ww, val in zip(ls_word, ls_y) if (
        val and np.abs(calDeg(x, y, ww['x1'], ww['y1'])-degree) < 3)]
    ...
    value_words = [val for val in value_words if len(
        val['label'].replace(' ', '').replace(':', '')) > 0]
    ...
    d = [levenshtein('darah', str(val['label']).lower())
    ...
    if(field_name == 'ttl'):
        d = [levenshtein('lahir', str(val['label']).lower())
             for val in value_words]
        ...
        d = [levenshtein('laki-laki', str(val['label']).lower())
             for val in value_words]
        if(len(d) > 0 and min(d) <= 2):
            return 'LAKI-LAKI'

        d = [levenshtein('laki', str(val['label']).lower())
             for val in value_words]
        if(len(d) > 0 and min(d) <= 1):
            return 'LAKI-LAKI'

        d = [levenshtein('wanita', str(val['label']).lower())
             for val in value_words]
        if(len(d) > 0 and min(d) <= 2):
            return 'WANITA'

        d = [levenshtein('perempuan', str(val['label']).lower())
             for val in value_words]
        if(len(d) > 0 and min(d) <= 2):
            return 'PEREMPUAN'
        ...
        d = [levenshtein('kartu', str(val['label']).lower())
             for val in value_words]
        if(len(d) > 0 and min(d) <= 2):
            idx = np.argmin(d)
            value_words.pop(idx)
        ...
        d = [levenshtein('wni', str(val['label']).lower())
             for val in value_words]
        if(len(d) > 0):
            return 'WNI'
        ...
        d = [levenshtein('hingga', str(val['label']).lower())
             for val in value_words]
        if(len(d) > 0 and min(d) <= 2):
            idx = np.argmin(d)
            value_words.pop(idx)
...
fields_ktp = [
    ...
    {'field_name': 'kewarganegaraan',
        'keywords': 'kewarganegaraan', 'typo_tolerance': 4},
...
def extract_ktp_data(text_response, debug_mode=False):
    ...
    for field in fields_ktp:
        field_value = get_attribute_ktp(
            ls_word, field['field_name'], field['keywords'], field['typo_tolerance'], debug_mode)
        if(field_value != None):
    ...
    if(attributes['identity_number'] != None):
        attributes['identity_number'] = ''.join(
            [i for i in raw_result['nik'] if i.isdigit()])
    ...
    if(raw_result['nama'] != None):
        attributes['fullname'] = ''.join(
            [i for i in raw_result['nama'] if not i.isdigit()]).replace('-', '').strip()
    ...
    if(attributes['birth_place'] != None):
        attributes['birth_place'] = ''.join(
            [i for i in attributes['birth_place'] if not i.isdigit()]).replace('-', '').replace('.', '').strip()
    ...
    if(raw_result['gol_darah'] != None):
        attributes['blood_type'] = ''.join(
            [i for i in raw_result['gol_darah'] if not i.isdigit()]).strip()
...
text = "ord —. Enis A --  -- yo = PROVINSI JAWA TIMUR --  -- 3 KABUPATEN SIDOARJO --  -- NIK : 3515084101030002 --  -- | Nama : ANANDA MUTIARA MAHARANI --  -- 4 TempauTgiLahir : SIDOARJO, 01-01-2003 --  -- j Jenis kelamin : PEREMPUAN Gol. Darah :O -- Alamat = : CITRA FAJAR GOLF E-7229 --  -- 1 RTRW : 002/008 -- : KevDesa : GEBANG -- Kecamatan :SIDOARJO ~ --  -- Agama : ISLAM -- Status Perkawinan: BELUM KAWIN “= -- Pekerjaan : BELUM/TIDAK BEKERJA --  -- Kewarganegaraan: WNI -- Berlaku Hingga  : SEUMUR HIDUP --  --"

ktp_extract = extract_ktp_data(text)
